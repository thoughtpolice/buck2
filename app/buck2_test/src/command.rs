...
async fn test(
    ...
    let explicit_tests: BuckMutSet<TargetLabel> = parsed_patterns_with_modifiers
        .iter()
    ...
    if let Some(internal_provider) = internal_provider {
        ...
        if use_internal_runner {
            let excluded = {
                let provider = internal_provider.as_ref().value().as_ref();
                label_filtering
                    .is_excluded(target.target().unconfigured(), provider.labels().collect())
            };
            if excluded {
                return Ok(None);
            }
...
struct TestLabelFiltering {
    ...
    explicit_tests: BuckMutSet<TargetLabel>,
}
