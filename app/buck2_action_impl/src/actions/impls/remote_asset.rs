...
impl RemoteAssetAction {
    ...
    async fn artifact_value(
        ...
        if self.inner.is_directory {
            ...
            let download_info = CasDownloadInfo::new_declared(self.inner.re_use_case);
            let root_directory = re_client
                .download_typed_blobs::<RE::Directory>(None, vec![digest.to_re()], &download_info)
                .await?
                .into_iter()
                .next()
                .ok_or_else(|| {
            ...
            let tree = re_directory_to_re_tree(root_directory, &re_client, &download_info).await?;
...
impl Action for RemoteAssetAction {
    ...
    async fn execute(
        ...
        let expires_at = response.expires_at.unwrap_or(Timestamp::UNIX_EPOCH);
