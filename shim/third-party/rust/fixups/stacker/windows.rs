// Vendored from stacker 0.1.25 (MIT OR Apache-2.0), byte-identical except for
// the `#[link]` attribute removed below. See fixups.toml for why.
...
//
// buck2 fork: upstream has `#[link(name = "stacker")]` here, naming the library
// `build.rs` would have produced. Build scripts do not run under reindeer, so
// the C file is compiled by the `stacker-c-msvc` cxx_library in fixups.toml and
// arrives on the link line by path; `-l stacker` would only send the MSVC
// linker looking for a `stacker.lib` that nothing produces (LNK1181).
extern "C" {
